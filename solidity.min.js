/*! `solidity` grammar compiled for Highlight.js 10.7.3 (0c4cc8a1) */
...
;const{baseAssembly:c,solAposStringMode:d,solQuoteStringMode:u,HEX_APOS_STRING_MODE:_,HEX_QUOTE_STRING_MODE:b,SOL_NUMBER:m,isNegativeLookbehindAvailable:g}=o
;return e=>{for(var a=d(e),s=u(e),n=[],i=0;i<32;i++)n[i]=i+1
;var t=n.map((e=>8*e)),r=[];for(i=0;i<=80;i++)r[i]=i
;var l=n.map((e=>"bytes"+e)).join(" ")+" ",o=t.map((e=>"uint"+e)).join(" ")+" ",E=t.map((e=>"int"+e)).join(" ")+" ",p=[].concat.apply([],t.map((e=>r.map((a=>e+"x"+a))))),M={
keyword:"var bool string int uint "+E+o+"byte bytes "+l+"fixed ufixed "+p.map((e=>"fixed"+e)).join(" ")+" "+p.map((e=>"ufixed"+e)).join(" ")+" enum struct mapping address unicode new delete if else for while continue break return throw emit try catch revert unchecked _ function modifier event constructor fallback receive error virtual override constant immutable anonymous indexed storage memory calldata external public internal payable pure view private returns import from as using global pragma contract interface library is abstract type assembly",
...
excludeBegin:!0,excludeEnd:!0,lexemes:C,keywords:M,
contains:[e.C_LINE_COMMENT_MODE,e.C_BLOCK_COMMENT_MODE,a,s,m,"self"]},f={
...
lexemes:C,keywords:M}),h={className:"built_in",
begin:(g()?"(?<!\\$)\\b":"\\b")+"(gas|value|salt)(?=:)"};function w(e,a){return{
begin:(g()?"(?<!\\$)\\b":"\\b")+e+"\\.\\s*",end:/[^A-Za-z0-9$_\.]/,
excludeBegin:!1,excludeEnd:!0,lexemes:C,keywords:{built_in:e+" "+a},
contains:[f],relevance:10}}var x=c(e),v=e.inherit(x,{
contains:x.contains.concat([{begin:/\./,end:/[^A-Za-z0-9$.]/,excludeBegin:!0,
...
keywords:M,lexemes:C,
contains:[a,s,_,b,e.C_LINE_COMMENT_MODE,e.C_BLOCK_COMMENT_MODE,m,h,O,{
className:"function",lexemes:C,
beginKeywords:"function modifier event constructor fallback receive error",
end:/[{;]/,excludeEnd:!0,
contains:[y,N,h,e.C_LINE_COMMENT_MODE,e.C_BLOCK_COMMENT_MODE],illegal:/%/
},w("msg","gas value data sender sig"),w("block","blockhash blobhash coinbase difficulty prevrandao gaslimit basefee blobbasefee number timestamp chainid"),w("tx","gasprice origin"),w("abi","decode encode encodePacked encodeWithSelector encodeWithSignature encodeCall"),w("bytes","concat"),w("string","concat"),f,{
...
},y,N,h,e.C_LINE_COMMENT_MODE,e.C_BLOCK_COMMENT_MODE]},{lexemes:C,
...
contains:[y,a,s,_,b,e.C_LINE_COMMENT_MODE,e.C_BLOCK_COMMENT_MODE,O]},{
